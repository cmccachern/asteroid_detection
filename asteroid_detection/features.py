...
def plot_rgb(r_fits, g_fits, b_fits, object_getter = asteroid):
    ...
    for ii, fits_file in enumerate([r_fits, g_fits, b_fits]):
        ...
        #data[ii] = np.log10(data[ii])

        #data[ii] = np.nan_to_num(data[ii])
...
data = data_from_fits("752_1_373_r.fits")
data = np.array(data)
...
def find_objects(img, threshold = .3):
    ...
    for ii in range(1,np.max(markers)):
        ...
        object_center = np.unravel_index(object_index, data.shape)
...
def find_asteroids(images, crop_width=50, crop_height=50):
    ...
    for obj in objects:
        ...
        if np.sum(np.abs(corl_max_rg)) > 1 and np.sum(np.abs(corl_max_ri)) > 1 and np.sum(np.abs(corl_max_rg - corl_max_ri)) > 1:
...
if __name__ == "__main__":
    ...
    fits = fits_from_rcf(run, camcol, field)
    images = {"r": fits["r"][0].data,
              "g": fits["g"][0].data,
              "i": fits["i"][0].data,
             }
    ...
    plt.figure()
    plt.imshow(np.log10(fits["r"][0].data))
